...
export async function validate(decoded: JwtPayload) {
  const user = await db.userStore.getUserById(decoded.id) as User;
  if (!user) {
...
export function getUserIdFromRequest(request: Request): string {
  ...
  try {
    ...
    const decodedToken = jwt.verify(token, "b20104109fitzpatrickHDipComputerScience") as jwt.JwtPayload;
